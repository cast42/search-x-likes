...
def ndcg_at_k(retrieved_docs: list[list[int]], ground_truths: list[int], k: int) -> float:
    ...
    scores: list[float] = []
    for i, retrieved in enumerate(retrieved_docs):
        relevance: list[int] = [1 if retrieved[j] == ground_truths[i] else 0 for j in range(k)]
        scores.append(ndcg_score([relevance], [list(range(k))]))  # Sklearn NDCG calculation
...
if __name__ == "__main__":
    ...
    mrr = mean_reciprocal_rank(queried_results, ground_truths)
    recall_5 = recall_at_k(queried_results, ground_truths, k=5)
    ndcg_5 = ndcg_at_k(queried_results, ground_truths, k=5)

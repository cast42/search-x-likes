...
if __name__ == "__main__":
    ...
    with timer("Reranking with Hugging Face Cross-Encoder"):
        for i, retrieved_ids in enumerate(queried_results):
            query = queries_lst[i]
            documents = [corpus_lst[doc_id] for doc_id in retrieved_ids]

            # Create pairs for the cross-encoder
            pairs = [[query, doc] for doc in documents]

            # Predict scores using the cross-encoder
            with torch.no_grad():  # disable gradient calculation to speed things up
                scores = model.predict(pairs, batch_size=32, show_progress_bar=False, num_workers=0)

            # Sort retrieved IDs based on cross-encoder scores
            doc_scores = list(zip(retrieved_ids, scores.tolist()))
